...
def draw_board_play(board_array, board_window):
    for i in range(27):
        board_window.addstr(3 + 4, (i * 2) + 2, "O" if board_array[i].game is None else board_array[i].game)
...
def move_player(players, board_window, player, board_array, amount):
    player.field = (player.field + amount) % 27
    draw_board_players(players, board_window)
    drawn_game = draw_random_game(board_array, board_window, player.field)
    return drawn_game

def draw_random_game(board_array, board_window, field):
    games = ["M", "J", "1"]
